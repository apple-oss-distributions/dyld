...
// BUILD(macos,ios,tvos,watchos): RC_XBS=NO $CC main.c -fsanitize=address  -o $BUILD_DIR/asan_launch_via_asan_libsystem_variant.exe
...
// RUN:  DYLD_PRINT_SEARCHING=1 DYLD_PRINT_LIBRARIES=1 DYLD_LIBRARY_PATH=/usr/appleinternal/lib/sanitizers DYLD_IMAGE_SUFFIX=_asan ./asan_launch_via_asan_libsystem_variant.exe
