...
// BUILD(macos,ios,tvos,watchos): RC_XBS=NO $CC main.c -fsanitize=address  -o $BUILD_DIR/asan_launch_via_executable_rpath.exe
// BUILD(macos,ios,tvos,watchos): $CP $asanDylibPath $BUILD_DIR/$asanDylibName
// RUN: DYLD_PRINT_SEARCHING=1 DYLD_PRINT_LIBRARIES=1 ./asan_launch_via_executable_rpath.exe
