...
int _start(const TestRunnerFunctions* hostFuncs)
{
    ...
    slideReturnCode = slide(hostFuncs->mhs[1], slideBasePointers, hostFuncs->printf);
    if ( slideReturnCode != 0 ) {
        FAIL("mhs[1] slide = %d\n", slideReturnCode);
        return 0;
    }
    ...
    slideReturnCode = slideKextsInsideKernelCollection(hostFuncs->mhs[1], slideBasePointers, hostFuncs->printf, hostFuncs);
    if ( slideReturnCode != 0 ) {
        FAIL("mhs[1] slide = %d\n", slideReturnCode);
        return 0;
    }
    ...
    int runModInitFuncs = runAllModInitFunctionsForAppCache(hostFuncs->mhs[1], hostFuncs->printf, hostFuncs);
