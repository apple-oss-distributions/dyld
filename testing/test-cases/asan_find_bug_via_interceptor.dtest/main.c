...
// BUILD(macos,ios,tvos,watchos): RC_XBS=NO $CC main.c -fsanitize=address  -o $BUILD_DIR/asan_find_bug_via_interceptor.exe
// BUILD(macos,ios,tvos,watchos): $DYLD_ENV_VARS_ENABLE $BUILD_DIR/asan_find_bug_via_interceptor.exe
// RUN:  DYLD_PRINT_SEARCHING=1 DYLD_PRINT_LIBRARIES=1 DYLD_LIBRARY_PATH=/usr/appleinternal/lib/sanitizers ./asan_find_bug_via_interceptor.exe
