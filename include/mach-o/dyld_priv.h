...
#if __has_include(<mach-o/dyld_version_defines.h>)
#include <mach-o/dyld_version_defines.h>
#endif
...
// called by libsystem_platform.dylib during libSystem_initializer()
...
// Returns true if the given address is in the shared cache.
// Sets the file path output parameter to the on disk path which contains the given address.
// Sets the fileOffset output parameter to the offset of the address in that file.
//
// Exists in 26.4 and later
extern bool _dyld_shared_cache_file_path_containing_address(const void* addr, char path[PATH_MAX], uint64_t* fileOffset);

// Call generated by linker to support lazy loading
// Support added in macOS/iOS 27.0
extern void _dyld_lazy_load(uint32_t* flag, const struct mach_header* mh);

// For use by TSan to get the address of dyld itself
// Added in macOS/iOS 26.4
