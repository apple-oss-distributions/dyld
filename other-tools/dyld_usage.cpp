...
static const std::string& stringForID(uint64_t id) {
...
struct output_renderer {
    ...
    _pid(ktrace_get_pid_for_thread(s, E->threadid)),
    _threadid((unsigned long)(E->threadid)) {}
...
private:
    ...
    struct dlsym : event_pair {
        dlsym(ktrace_event_t E) : event_pair(E), symbol(stringForID(E->arg3)), handle(E->arg2) {}
        std::string symbol;
        uint64_t handle;
        uint64_t result;
    };
