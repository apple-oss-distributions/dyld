...
// Swift implementation of iterate_text (avoids ObjC bridging overhead)
extern "C" int32_t dyld_shared_cache_iterate_text_swift(const uint8_t* cacheUuid,
                                                         void (^callback)(const dyld_shared_cache_dylib_text_info*));
extern "C" int32_t dyld_shared_cache_find_iterate_text_swift(const uint8_t* cacheUuid,
...
void dyld_for_each_installed_shared_cache(void (^block)(dyld_shared_cache_t cache)) {
    NSArray<_DYSharedCache *>* caches = [_DYSharedCache systemSharedCaches];
...
void dyld_for_each_installed_shared_cache_ex(bool includeNonSystemCaches, void (^block)(dyld_shared_cache_t cache, bool* stop)) {
    NSArray<_DYSharedCache *>* caches = [_DYSharedCache systemSharedCaches];
    bool stop = false;
    for (_DYSharedCache* cache in caches) {
        @autoreleasepool {
            dyld_shared_cache_t cacheObjc = (__bridge dyld_shared_cache_t)cache;
            block(cacheObjc, &stop);
        }
        if (stop) { return; }
    }

    if (!includeNonSystemCaches) { return; }
    caches = [_DYSharedCache installedSharedCaches];
    for (_DYSharedCache* cache in caches) {
        @autoreleasepool {
            dyld_shared_cache_t cacheObjc = (__bridge dyld_shared_cache_t)cache;
            block(cacheObjc, &stop);
...
void dyld_for_each_installed_shared_cache_with_system_path(const char* rootPath, void (^block)(dyld_shared_cache_t cache)) {
    ...
    NSArray<_DYSharedCache *>* caches = [_DYSharedCache systemSharedCachesForSystemPath:[NSString stringWithUTF8String:rootPath]];
    for (_DYSharedCache* cache in caches) {
        @autoreleasepool {
            dyld_shared_cache_t cacheObjc = (__bridge dyld_shared_cache_t)cache;
            block(cacheObjc);
...
bool dyld_image_content_for_segment(dyld_image_t image, const char* segmentName,
    ...
    for (_DYSegment* segment in imageObjc.segments) {
        if (strcmp(segment.name.UTF8String, segmentName) == 0) {
            dispatch_data_t segmentData = segment.data;
            if (segmentData == NULL) { return false; }
            const void* segmentBytes = nullptr;
            (void)dispatch_data_create_map(segmentData, &segmentBytes, nil);
            @autoreleasepool {
                contentReader(segmentBytes, segment.address, segment.vmsize);
            }
            return true;
        }
    }
    // We shoukd return true because we were able to correctly materialize the image, but we don't run the content reader because
    // they asked for a non-existent segment. Unfortunately our clients depend on this being false because we were not explicit about it before.
    return false;
}
...
bool dyld_image_for_each_section_info(dyld_image_t image, void (^block)(const char* segmentName, const char* sectionName, uint64_t vmAddr, uint64_t vmSize)) {
    ...
    dispatch_data_t textSegmentData = textSegment.data;
    if (textSegmentData == NULL) { return false; }
    const void* textSegmentBytes = nullptr;
    size_t      textSegmentSize = 0;
    (void)dispatch_data_create_map(textSegmentData, &textSegmentBytes, &textSegmentSize);
    std::span<const uint8_t> mhSpan((const uint8_t*)textSegmentBytes, textSegmentSize);
    if ( const mach_o::UnsafeHeader* mh = mach_o::UnsafeHeader::isMachO(mhSpan) ) {
        mh->forEachSection(^(const mach_o::UnsafeHeader::SectionInfo& info, bool& stop) {
...
bool dyld_image_content_for_section(dyld_image_t image, const char* segmentName, const char* sectionName,
    ...
    std::span<const uint8_t> mhSpan((const uint8_t*)textSegmentBytes, textSegmentSize);
    if ( const mach_o::UnsafeHeader* mh = mach_o::UnsafeHeader::isMachO(mhSpan) ) {
        __block bool sectionFound = false;
        mh->forEachSection(^(const mach_o::UnsafeHeader::SectionInfo& info, bool& stop) {
        ...
        if (!sectionFound) {
            // We shoukd return true because we were able to correctly materialize the image, but we don't run the content reader because
            // they asked for a non-existent section. Unfortunately our clients depend on this being false because we were not explicit about it before.
            return false;
        }
    } else {
        return false;
    ...
    dispatch_data_t hostSegmentData = hostSegment.data;
    if (hostSegmentData == NULL) { return false; }
    const void* hostSegmentBytes = nullptr;
    size_t      hostSegmentSize = 0;
    (void)dispatch_data_create_map(hostSegmentData, &hostSegmentBytes, &hostSegmentSize);
    @autoreleasepool {
        contentReader((const void*)(((uint8_t*)hostSegmentBytes)+sectionOffset), sectionOffset+hostSegment.address, sectionSize);
    }
...
bool dyld_image_local_nlist_content_4Symbolication(dyld_image_t image,
    ...
    [sharedCache withLocalSymbolData:^(const void * _Nonnull buffer, size_t size) {
        uint64_t textOffsetInCache = imageObjc.address - sharedCache.address;
        const dyld_cache_header* header = (const dyld_cache_header*)buffer;
        const dyld_cache_local_symbols_info* localInfo = (dyld_cache_local_symbols_info*)((const uint8_t*)buffer + header->localSymbolsOffset);
...
bool dyld_shared_cache_for_each_subcache4Rosetta(dyld_shared_cache_t cache, void (^block)(const void* cacheBuffer, size_t size)) {
    ...
    // This is OS X only so we can we have enoguh address space, and since we need to access each sub cache in VM layout anyway
    // lets just pin the whole thing and walk through it, that wat we don't need have special code for mapping subcaches, etc
    _DYSharedCache* sharedCacheObjc = (__bridge _DYSharedCache*)cache;
    [sharedCacheObjc pinMappings];
    for (_DYSubCache* subCache in sharedCacheObjc.subCaches) {
        __block bool done = false;
        @autoreleasepool {
            [subCache withVMLayoutBytes:^(const void * _Nullable buffer, size_t size) {
                const dyld_cache_header* header = (const dyld_cache_header*)buffer;
                if (strcmp(header->magic, "dyld_v1  x86_64") != 0) {
                    done = true;
                    return;
                }
                block((const void*)buffer, size);
            }];
        }
        if (done) { break; }
    }
    [sharedCacheObjc unpinMappings];
    return true;
}
#endif /* TARGET_OS_OSX */
...
void _dyld_process_info_for_each_image(dyld_process_info info, void (^callback)(uint64_t machHeaderAddress, const uuid_t uuid, const char* path)) {
    ...
    for (_DYImage* image in snapshot.images) {
        @autoreleasepool {
            ...
            const char* filePath = image.filePath.UTF8String;
            if (filePath) {
                strlcat(path, filePath, (size_t)MIN(MAXPATHLEN, prefixSize+strlen(filePath)+1));
            } else {
                const char* installname = image.installname.UTF8String;
                if (installname) {
                    strlcat(path, installname, (size_t)MIN(MAXPATHLEN, prefixSize+strlen(installname)+1));
                } else {
                    pathPtr = "<bad_string>";
                }
            }
            ...
            callback(image.address, uuid, path);
        }
    }
}
...
dispatch_data_t dyld_image_segment_data_4HWTrace(dyld_image_t image, const char* segmentName) {
    _DYImage* objc = (__bridge _DYImage*)image;
    for (_DYSegment* segment in objc.segments) {
        if (strcmp(segment.name.UTF8String, segmentName) == 0) {
            return segment.data;
        }
    }
...
int dyld_shared_cache_iterate_text(const uuid_t cacheUuid, void (^callback)(const dyld_shared_cache_dylib_text_info* info)) {
    // Use pure Swift implementation to avoid ObjC bridging overhead
    return dyld_shared_cache_iterate_text_swift(cacheUuid, callback);
}
...
int dyld_shared_cache_find_iterate_text(const uuid_t cacheUuid, const char* extraSearchDirs[], void (^callback)(const dyld_shared_cache_dylib_text_info* info)) {
    // Use pure Swift implementation to avoid ObjC bridging overhead
    return dyld_shared_cache_find_iterate_text_swift(cacheUuid, extraSearchDirs, callback);
}
...
void* dyld_process_snapshot_create_metrics(dyld_process_snapshot_t snapshot) {
    ...
    _DYSnapshot* snapshotObjc = (__bridge _DYSnapshot*)snapshot;
    return (void*)CFBridgingRetain(snapshotObjc.metrics);
}
