...
// This can be HW accelerated if it ever proves to be an issue
extension Collection where Element == UInt8 {
    func cksum() -> UInt32 {
        var count = UInt32(self.count)
        var crc = reduce(into: 0) { accum, byte in
            accum = (accum << 8) ^ CRCTable[Int((accum >> 24) ^ UInt32(byte))];
        }
        // In addition to the file content, cksum appends the octets of the size and uses them int he crc calculation
        while count != 0 {
            crc = (crc << 8) ^ CRCTable[Int((crc >> 24) ^ UInt32(count & 0xff))];
            count = count >> 8
        }
        return ~crc
  }
