...
env -i DEVELOPER_DIR=${DEVELOPER_DIR} xcrun -v -sdk macosx.internal clang++ -target arm64-apple-macos13.0 -DBUILDING_DYLD=1 -std=c++2a -w -Wno-incompatible-sysroot -fsyntax-only -Xclang -fdump-record-layouts -Icommon -Idyld -Iinclude -Icache_builder -Ilsl -Imach_o -Iinclude/mach-o ${DERIVED_FILE_DIR}/test.cpp >${DERIVED_FILE_DIR}/test.out
