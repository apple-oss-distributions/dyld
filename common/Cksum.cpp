...
// We don't use the HW intrinsics for crc32 because those use reflected inputs, and cksum does not. If this ever shows up in profiles
// we can implement it via pmull intrinsics instead of using the lookup table
uint32_t cksum(const std::span<std::byte> bytes) {
    size_t size = bytes.size();
    uint32_t result = 0;
    // CRC the data
    for (uint32_t i = 0; i < size; ++i) {
        result = (result << 8) ^ crcTable[(result >> 24) ^ (uint32_t)bytes[i]];
    }
    // CRC the octets of the size field
    for(; size != 0; size >>= 8) {
        result = (result << 8) ^ crcTable[(result >> 24) ^ (size & 0xff)];
    }
    // Invert the results
    return ~result;
}
